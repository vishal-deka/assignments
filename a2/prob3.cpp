...
pthread_mutex_t n2 = PTHREAD_MUTEX_INITIALIZER; /* mutex for 2nd file */
...
void *producer(void *param)
{
    ...
    while (1)
    {
        pthread_mutex_lock(&n); // lock file
        if (is.eof()) {
        ...
        pthread_mutex_unlock(&n);   //unlock file

        pthread_mutex_lock(&m);     //lock buffer
        if (num > BUF_SIZE)
            exit(1);
        ...
        pthread_mutex_unlock(&m);   //unlock bufer
...
void *consumer(void *param)
{
    ...
    while (1)
    {
        ...
        pthread_mutex_lock(&n2);    //lock file
        if (isPrime(i))
        {
            os<<i<<std::endl;
        }
        pthread_mutex_unlock(&n2);  //lock file
